...
public class AnalyticalJSON {
    ...
    public static ArrayList<? extends MultiItemEntity> getItemEntitys(String json) {
        ArrayList<Zixun_Tougao.MyItem> list = new ArrayList<>();
        try {
            JSONArray jsonArray = new JSONArray(json);
                for (int i = 0; i < jsonArray.length(); i++) {
                    JSONObject js = jsonArray.getJSONObject(i);
                    Zixun_Tougao.MyItem item = new Zixun_Tougao.MyItem();
                    if ("".equals(js.getString("options"))) {
                        item.setItemType(Zixun_Tougao.TGAdapter.TEXT);
                    } else if (js.getString("options").endsWith("mp3")){
                        item.setItemType(Zixun_Tougao.TGAdapter.AUDIO);
                    }else if (js.getString("options").endsWith("mp4")){
                        item.setItemType(Zixun_Tougao.TGAdapter.VIDEO);
                    }
                    item.setTitle(js.getString("title"));
                    item.setTime(js.getString("time"));
                    item.setId(js.getString("id"));
                    item.setComment(js.getString("draft_comment"));
                    item.setCtr(js.getString("ctr"));
                    item.setHeadUrl(js.getString("user_image"));
                    item.setLike(js.getString("likes"));
                    item.setOptions(js.getString("options"));
                    item.setPetName(js.getString("pet_name"));
                    item.setThumbUrl(js.getString("image1"));
                    item.setUser_id(js.getString("user_id"));
                    list.add(item);
                }
